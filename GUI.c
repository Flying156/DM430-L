...
uchar Taby1[][7]={"26","27","28","29","30","31","32"} ; //y刻度值，可自由定义
uchar Taby2[][7]={"79","81","83","85","87","89","88"} ;
...
* 出口参数：无 
****************************************************************************/ 
void GUICoordinate1(uint x0,uint y0,uint xm,uint ym,uchar pix,uint ccolor,uint bcolor) 
{ 
  uchar i,xj,yj;
  ...
  LCD_PutString(x0+10*cos(1*PI/3),y0-ym+10*sin(1*PI/3),"Temperature",ccolor,bcolor);      //显示Y轴参数定义，如电压V
  ...
  for(i=1;i<yj;i++)
  {
    ...
    LCD_PutString(x0-5-24,y0-i*ym/yj-8,Taby1[i],ccolor,bcolor);  //显示刻度参数值，定义在Tab表里
  }
...
void GUICoordinate2(uint x0,uint y0,uint xm,uint ym,uchar pix,uint ccolor,uint bcolor) 
{ 
  uchar i,xj,yj;
  ...
  for(i=1;i<yj;i++)
  {
    ...
    LCD_PutString(x0-5-24,y0-i*ym/yj-8,Taby2[i],ccolor,bcolor);  //显示刻度参数值，定义在Tab表里
  }
